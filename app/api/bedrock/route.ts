...
export async function POST(request: NextRequest) {
    try {
        const { prompt, modelId = 'arn:aws:bedrock:eu-west-1:501010171970:inference-profile/eu.anthropic.claude-sonnet-4-20250514-v1:0' } = await request.json();

        // Use standard model if inference profile is not accessible
        const actualModelId = modelId.includes('inference-profile') ? modelId : 'anthropic.claude-3-5-sonnet-20241022-v1:0';
